...
{
    SetNbNkNr(keysize); //设置密钥块数，轮数//lt:keysize=16,keybytes=key-ole-abcdefgh
    memcpy(key,keyBytes,keysize);//lt:外部key被屏蔽                //字符串拷贝函数，把keyBytes的keysize个字符复制到key中
...
VOID TAesClass::InitializePrivateKey(DWORD KeySize,UCHAR *KeyBytes)
{
    ...
    m_lpAes=new Aes(KeySize,KeyBytes);//lt:16,key-ole-abcdefgh
